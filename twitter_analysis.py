...
def main():
    ...
    plt.savefig('tweets_by_country', format='png')
    ...
    prg_langs = ['python', 'javascript', 'ruby', 'java']
    tweets_by_prg_lang = [tweets['python'].value_counts()[True], tweets['javascript'].value_counts()[True],
                          tweets['ruby'].value_counts()[True], tweets['java'].value_counts()[True]]
    x_pos = list(range(len(prg_langs)))
    width = 0.6
    fig, ax = plt.subplots()
    plt.bar(x_pos, tweets_by_prg_lang, width, alpha=1, color='g')
    ax.set_ylabel('Number of tweets', fontsize=15)
    ax.set_title('Ranking: python vs. javascript vs. ruby vs. java (Raw data)', fontsize=10, fontweight='bold')
    ...
    plt.bar(x_pos, tweets_by_prg_lang, width, alpha=1, color='g')
    ax.set_ylabel('Number of tweets', fontsize=15)
    ax.set_title('Ranking: python vs. javascript vs. ruby vs. java (Relevant data)', fontsize=10, fontweight='bold')
    ax.set_xticks([p + 0 * width for p in x_pos])
    ax.set_xticklabels(prg_langs)
    # plt.grid()
    ...
    print('\nBelow are some Python links that were extracted\n')
